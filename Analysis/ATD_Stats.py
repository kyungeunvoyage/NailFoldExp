...
def plot_accuracy_contrast_panel(ax, plot_df, lme_by_label, title):
    ...
    ax.axhline(80, color="#c75b5b", linestyle="--", linewidth=1, alpha=0.55)
    ...
    for i, cv in enumerate(order):
        ...
        if r is None:
            ax.text(i, y_ann, "LME fail", ha="center", fontsize=8, color="gray")
        else:
            ax.text(
                i,
                y_ann,
...
if not all_files:
    ...
    # --- Lateral / proximal: accuracy (y) by contrast; LME p for first−second area ---
    ...
    fig_lp, (ax_l, ax_r) = plt.subplots(
        1, 2, figsize=(14.5, 5.2), gridspec_kw={"wspace": 0.28}
    )
    fig_lp.suptitle(
    ...
    fig_lp.savefig(out_lp, dpi=220, bbox_inches="tight", facecolor="white")
    print(f"\nSaved: {out_lp}")
