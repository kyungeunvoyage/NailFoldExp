...
corr_2_by_force.py
==================
Force별 Relative_Score × 손톱 비율 (W:H, L/W, R/W) Correlation

실행: python corr_2_by_force.py
"""
...
force_list = sorted(df['Force_Val'].unique())
print(f"Force 종류: {force_list}")

# Force별 피험자 평균
atd_force = (df.groupby(['SubjectID','Force_Val'])['Relative_Score']
             .mean().reset_index()
...
# 병합
df_m = atd_force.merge(df_nail[['subject_id']+NAIL_VARS],
                        left_on='SubjectID', right_on='subject_id', how='inner')
n_subjects = df_m['SubjectID'].nunique()
print(f"병합 완료: {n_subjects}명")

# ══════════════════════════════════════════════════════════════════
# Plot A: Scatter grid (Force × nail var)
# ══════════════════════════════════════════════════════════════════
n_forces = len(force_list)
n_nails  = len(NAIL_VARS)

fig, axes = plt.subplots(n_forces, n_nails,
                          figsize=(5*n_nails, 4*n_forces),  # 행/열 수에 따라 자동
                          squeeze=False)

for row_i, fv in enumerate(force_list):
    sub = df_m[df_m['Force_Val'] == fv]
    ...
    for col_j, nv in enumerate(NAIL_VARS):
        ...
        ax.set_title(f'Force {fv}g  ×  {NAIL_LABELS[nv]}\nr={r_p:+.3f} {s_label}  ρ={r_s:+.3f} {star(p_s)}',
                     color=tc, fontsize=9, pad=6)
        ax.set_xlabel(NAIL_LABELS[nv], fontsize=8)
        ax.set_ylabel(f'Score @ {fv}g (%)', fontsize=8)
        ax.grid(zorder=0)

fig.suptitle(f'[2] ATD Score by Force × Nail Ratios  (n={n_subjects})\nGreen = significant (p<.05)',
             color=ACCENT, fontsize=13, y=1.01)
plt.tight_layout()
path = os.path.join(OUTPUT_DIR, '2_force_scatter_grid.png')
fig.savefig(path, dpi=150, bbox_inches='tight', facecolor='#0f0f0f')
print(f"저장 → {path}")
plt.show()

# ══════════════════════════════════════════════════════════════════
# Plot B: Heatmap — Force × nail var (r값)
# ══════════════════════════════════════════════════════════════════
r_mat   = pd.DataFrame(index=[f'{fv}g' for fv in force_list],
...
for fv in force_list:
    sub = df_m[df_m['Force_Val'] == fv]
    for nv in NAIL_VARS:
        tmp = sub[['score', nv]].dropna()
        if len(tmp) < 4:
            r_mat.loc[f'{fv}g', NAIL_LABELS[nv]]   = np.nan
            ann_mat.loc[f'{fv}g', NAIL_LABELS[nv]] = ''
            continue
        r_p, p_p = stats.pearsonr(tmp['score'], tmp[nv])
        r_mat.loc[f'{fv}g', NAIL_LABELS[nv]]   = round(r_p, 3)
        s = star(p_p) if p_p < 0.05 else ''
        ann_mat.loc[f'{fv}g', NAIL_LABELS[nv]] = f'{r_p:+.3f}\n{s}'
...
ax.set_title(f'Pearson r — Score by Force × Nail Ratios  (n={n_subjects})\n* p<.05  ** p<.01  *** p<.001',
             color=ACCENT, pad=10)
ax.set_yticklabels(ax.get_yticklabels(), rotation=0)
ax.set_xticklabels(ax.get_xticklabels(), rotation=15)

path2 = os.path.join(OUTPUT_DIR, '2_force_heatmap.png')
fig.savefig(path2, dpi=150, bbox_inches='tight', facecolor='#0f0f0f')
print(f"저장 → {path2}")
plt.show()

# ══════════════════════════════════════════════════════════════════
# Plot C: r값 라인 플롯 — Force에 따른 correlation 변화
# ══════════════════════════════════════════════════════════════════
...
for nv in NAIL_VARS:
    r_vals, p_vals = [], []
    for fv in force_list:
        sub = df_m[df_m['Force_Val'] == fv][['score', nv]].dropna()
        if len(sub) >= 4:
    ...
    c = nail_colors[nv]
    ax.plot(force_list, r_vals, color=c, lw=2, marker='o', markersize=8,
            label=NAIL_LABELS[nv], zorder=3)
    # 유의미한 포인트 강조
    for fv, r, p in zip(force_list, r_vals, p_vals):
        if p is not None and p < 0.05:
            ax.scatter(fv, r, color=c, s=150, zorder=5, edgecolors='white', linewidths=1.5)
            ax.annotate(star(p), (fv, r), xytext=(0, 10), textcoords='offset points',
...
ax.axhline(-0.3, color='white', lw=0.5, ls=':', alpha=0.2)
ax.set_xlabel('Stimulus Force (g)')
ax.set_ylabel('Pearson r')
ax.set_title(f'[2] Correlation Change by Force  (n={n_subjects})\nLarge dot = significant (p<.05)',
             color=ACCENT, pad=10)
ax.legend(fontsize=9, facecolor='#1a1a1a', edgecolor='#444')
ax.set_xticks(force_list)
ax.set_ylim(-1, 1)
ax.grid(zorder=0)

path3 = os.path.join(OUTPUT_DIR, '2_force_r_lineplot.png')
