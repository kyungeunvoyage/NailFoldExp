...
FIG2_BRACKET_TICK_H = 0.5
FIG2_BRACKET_TEXT_PAD = 0.0
...
def add_legend_outside(
    ...
    *,
    ...
    # tuple handles (HandlerTuple): take label from first element
    labels = [
    ...
    if handler_map is not None:
        leg_kw["handler_map"] = handler_map
    ax.legend(**leg_kw)
...
def _add_condition_sig_bracket(ax, x_l, x_r, y_base, text="", tick_h=FIG2_BRACKET_TICK_H):
    """Horizontal significance line between dodged boxes (no vertical ticks)."""
...
def plot_kao_vs_periungual(df_periungual, peri_label, peri_color, save_stem,
    ...
    kao_patch  = mpatches.Patch(facecolor="none", edgecolor=KAO_COLOR,
                                linewidth=KAO_BOX_LINEWIDTH, label=KAO_LABEL)
    peri_patch = mpatches.Patch(facecolor=peri_box_face,
                                edgecolor=BLACK, linewidth=BOX_LINEWIDTH,
                                label=peri_label)

    leg_handles = [kao_patch, peri_patch]
